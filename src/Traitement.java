...
/*
 This class identify the chains (liagnds, protein...).
 */
...
public class Traitement {
	...
	public Traitement(Molecule mol) {
		this.mol = mol;
		...
		/*
		 * if proteic chain has less than 11 aa it put as ligand (3 for nucleic
		 * acids)
		 */
		trait2();

		/* concatenation of chain with same name and same type */
		trait3();

		updateConect();

		updateConectRes();
		/* for ligands test connectivity and split if the chain is broken */
		trait4();
	...
	public void trait1() {
		...
		for (int i = 0; i < l.size(); i++) {
			...
			if (c.getNatoms() == c.getHetero())
				...
				if ((c.getName() == ' ') && (b == true) && (!containStruc(c)))
					c.setType("ligand");
				else {
	...
	/* use only for tests */
	public void deleteBond(int id1, int id2) {
		for (int v = 0; v < mol.chains.size(); v++) {
			Chain c = (Chain) mol.chains.get(v);
			if (c.getType().equals("ligand")) {
				LinkedList residues = c.getResidues();
