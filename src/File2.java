...
/*
 This class read a PDB file, create the molecule with only the ligand given by the parameters.
 */
...
public class File2 {
	...
	public File2(String file, int res1, int res2, char iden) {
		this.file = file;
		this.res1 = res1;
		this.res2 = res2;
		this.iden = iden;
	...
	public Molecule getMolecule() throws IOException {
		...
		if (file.indexOf(".pdb") == -1) {
			...
			try {
				...
				//URL monUrl = getClass().getResource(file);
				//InputStreamReader ins = new InputStreamReader(monUrl.openStream());
				//BufferedReader buff = new BufferedReader(ins);
				...
				try {
					...
					mol.setProp(prop);
					mol.setName(file.substring(0, file.indexOf(".")));

					Chain chain = new Chain();
					chain.setParent(mol);
					chain.setName(iden);
					chain.setId(0);
					chain.setType("ligand");
					mol.addChain(chain);
						...
						if (!line.equals("")) {
							...
							if (c0 == 'C' && c1 == 'O' && c2 == 'N'
								...
								if (line.charAt(10) != ' ') {
									...
									debut = zId;
									// System.out.println("CONECT "+debut);
								}
							...
							if ((seenENDMDL == false)
								...
								if ((alt == ' ') || (alt == 'A')) {
									...
									String tempo = "";
									if (line.charAt(22) != ' ')
										tempo = tempo + line.charAt(22);
									if (line.charAt(23) != ' ')
										tempo = tempo + line.charAt(23);
									if (line.charAt(24) != ' ')
										tempo = tempo + line.charAt(24);
									if (line.charAt(25) != ' ')
										tempo = tempo + line.charAt(25);
									if (line.charAt(26) != ' ')
										tempo = tempo + line.charAt(26);
									String residueIdo = tempo;

									int resIdInt;
									String tp1 = residueIdo;
									String ttp1 = "";
									...
									resIdInt = Integer.parseInt(ttp1);

									char id = line.charAt(21);
									...
									if ((!isWater(residueName))
										...
										if (((resIdInt >= res1) && (resIdInt <= res2))
												&& (iden == chainId)) {

											currentChain = chain;

											if (chain.containRes(residueId)) {
											} else {
												Residue res = new Residue();
												res.setName(residueName);
												res.setId(residueId);
												res.setParent(chain);
												mol.addResidue(0, res);
												currentRes = res;
											...
											if (chainId == currentId) {
												...
												nChain = nChain + 1;
												Chain chainC = new Chain();
												chainC.setName(chainId);
												chainC.setId(nChain);
												chainC.setParent(mol);
												mol.addChain(chainC);
												currentId = chainId;
												currentChain = chainC;
												curId = nChain;
											...
											if (currentChain
												...
												if (res.isIon(currentChain)) {
													...
													currentChain2 = chainC;
													res.setParent(chainC);
